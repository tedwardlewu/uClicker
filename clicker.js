document.addEventListener('DOMContentLoaded', function() {
    ...
    elements.toggle.addEventListener("change", () => {
        ...
        if (isOnIclickerPage) {
            sendMessage({ action: "toggle", enabled })
    ...
    function refreshStatus() {
        ...
        sendMessage({ action: "getStatus" })
            .then(status => setConnectedState(status))
            .catch(error => setDisconnectedState(error));
    }
        ...
        return new Promise((resolve, reject) => {
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                ...
                chrome.tabs.sendMessage(tabs[0].id, msg, (response) => {
                    if (chrome.runtime.lastError) reject("Content script not ready");
    ...
    function setConnectedState(status) {
        ...
        updateCard('pollCard', status.pollActive);
        updateCard('activityCard', status.pollActive);
    ...
    function updateCard(cardId, active) {
        ...
        if (card) card.className = `status-card ${active ? 'active-alert' : 'inactive'}`;
    }
    ...
    function setDisconnectedState(error) {
        ...
        ['pollCard', 'activityCard'].forEach(id => updateCard(id, false));
        elements.connectionStatus.className = "connection-status disconnected";
        elements.connectionStatus.title = `Error: ${error}`;
    }

    function sendDesktopNotification(title, msg) {
        if (Notification.permission === "granted") new Notification(title, { body: msg, icon: "icon.png" });
        else if (Notification.permission !== "denied") {
            Notification.requestPermission().then(p => {
                if (p === "granted") new Notification(title, { body: msg, icon: "icon.png" });
            });
        }
    }
